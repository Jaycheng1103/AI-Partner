...
To install separately, copy this entire folder, including `assets`, `scripts`, `references`, and `agents`, to the target AI Partner's `.agents/skills/3d-brain/`. In AI Partner, run `bash scripts/sync-codex-skills.sh 3d-brain` to generate the Codex copy. Do not copy only `SKILL.md` or include a generated user's config/data.
