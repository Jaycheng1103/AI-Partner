...
獨立安裝時，把整個技能資料夾（包括 `assets`、`scripts`、`references`、`agents`）複製到目標系統的 `.agents/skills/3d-brain/`。在本套件中執行 `bash scripts/sync-codex-skills.sh 3d-brain`，生成 Codex 副本。不能只複製 `SKILL.md`，也不要夾帶已生成的個人設定或資料。
